...
void OpenGLWidget::paintGL()
{
    ...
    for(int i=0; i<estrelas; ++i)
    {
        glUniform4f(locTranslation, posicObjet[i].x(), posicObjet[i].y(), posicObjet[i].z(), 0);
...
void OpenGLWidget::animate()
{
    ...
    for(int i=0; i<estrelas; ++i)
    {
        ...
        QVector3D pos = posicObjet[i];
        pos.setZ(pos.z() + a - elapsedTime / 50);
        if (pos.z() <= -1.0f)
        {
            float ang = (qrand() / (float)RAND_MAX) * 2 * 3.14159265f;
            float radius = 1 + (qrand() / (float)RAND_MAX) * 2;
...
void OpenGLWidget::createVBOs()
{
    ...
    vertices[0] = QVector4D( 0.0, -1.00, 0, 1) * scale;
    vertices[1] = QVector4D(-0.75,  0.5, 0, 1) * scale;
    vertices[2] = QVector4D( 0.75,  0.5, 0, 1) * scale;
    vertices[3] = QVector4D( 0.0,  1.00, 0, 1) * scale;
    vertices[4] = QVector4D(-0.75, -0.5, 0, 1) * scale;
    vertices[5] = QVector4D( 0.75, -0.5, 0, 1) * scale;
